...
struct sysconf_t * initial_sysconf_module (char *file,
	...
	char			*cfgfile;
#ifdef x__BORLANDC__
	static struct preset_sysconf_t {
		char	*key;
		int	type;
		char	*value;
	} preset[] = {
		{ "student-score-file"    , 0, "score.txt"   	},
		{ "student-wishes-file"   , 0, "wish.txt"    	},
		{ "student-info-file"	  , 0, "stdinfo.txt"	},
		{ "department-limit-file" , 0, "deplim.dat"  	},
		{ "standard-file"         , 0, "standard.txt"	},
		{ "output-file"		  , 0, "result.txt"	},
		{ "check-file"		  , 0, "check.txt"	},
                { "deny-list-file"        , 0, "denylist.txt"   },
		{ "skill-set-file"	  , 0, "skillset.txt"	},
		{ "print-level"		  , 1, "5"		},
		{ "output-format"	  , 1, "1"		},
		{ NULL			  , 0, NULL		}
	};
...
#elif defined (x__BORLANDC__)
	for (i = 0; preset[i].key != NULL; i++) {
		if (preset[i].type == 1) {
			syscnf.addint (preset[i].key, preset[i].value);
		} else {
			syscnf.addstr (preset[i].key, preset[i].value);
		}
	}
#endif
